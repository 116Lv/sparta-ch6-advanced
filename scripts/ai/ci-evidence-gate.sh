...
if [ "$#" -ne 0 ] || [ "$task_key_set" -ne 1 ] || [ "$gate_invocation_id_set" -ne 1 ] || [ "$output_set" -ne 1 ]; then
  printf '%s\n' '{"$schema":"ai/schemas/ci-gate-result.schema.json","$id":"ai/ci-gate-result.json","schemaVersion":1,"operation":"CI_EVIDENCE_GATE","result":"BLOCKED","phase2cLeafResult":"BLOCKED","reason":"INVALID_CI_GATE_ARGUMENTS","data":{"taskKey":"invalid","gateInvocationId":"invalid","requiredCheck":"phase-3b-ci-gates","provider":"github-actions","workflowRefs":[],"nativeAdapterInstallation":{"status":"NOT_CONFIGURED","reasonCode":"INVALID_CI_GATE_ARGUMENTS"},"durableEvidence":{"status":"NOT_CONFIGURED","retentionDays":90,"artifactRefs":[]},"remoteRunner":{"status":"NOT_CONFIGURED","completionBlocking":true,"reasonCode":"INVALID_CI_GATE_ARGUMENTS"},"nativeAdapterLeaf":{"nativeAdapterCheckId":"native-runtime-adapter","currentHostResult":"UNSUPPORTED"},"cachePolicy":{"reuse":"FORBIDDEN_WITHOUT_MATCHING_RUN_ID","handoff":"SUMMARY_ONLY"},"phase2CLeafResult":"BLOCKED"}}'
  exit 2
fi
...
printf '%s\n' '{"$schema":"ai/schemas/ci-gate-result.schema.json","$id":"ai/ci-gate-result.json","schemaVersion":1,"operation":"CI_EVIDENCE_GATE","result":"NOT_CONFIGURED","phase2cLeafResult":"BLOCKED","reason":"HELPER_RUNTIME_UNAVAILABLE","data":{"taskKey":"runtime-unavailable","gateInvocationId":"runtime-unavailable","requiredCheck":"phase-3b-ci-gates","provider":"github-actions","workflowRefs":[],"nativeAdapterInstallation":{"status":"NOT_CONFIGURED","reasonCode":"HELPER_RUNTIME_UNAVAILABLE"},"durableEvidence":{"status":"NOT_CONFIGURED","retentionDays":90,"artifactRefs":[]},"remoteRunner":{"status":"NOT_CONFIGURED","completionBlocking":true,"reasonCode":"HELPER_RUNTIME_UNAVAILABLE"},"nativeAdapterLeaf":{"nativeAdapterCheckId":"native-runtime-adapter","currentHostResult":"UNSUPPORTED"},"cachePolicy":{"reuse":"FORBIDDEN_WITHOUT_MATCHING_RUN_ID","handoff":"SUMMARY_ONLY"},"phase2CLeafResult":"BLOCKED"}}'
exit 3
