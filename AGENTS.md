...
Crash-left `FINALIZING` or receipt-bearing `FINALIZED` runs must be reconciled through `scripts/ai/done-claim-check.sh recover-finalization <run-id>`; recovery resumes only a valid sealed receipt, otherwise it removes the exact owned finalization refs and restores `OPEN`, and it never rolls back after `run.json` exists.
