...
Phase 3A handoffs also include `ai/native-runtime-adapters.json`, `ai/native-runtime-adapters.md`, `ai/schemas/native-runtime-snapshot.schema.json`, and Issue #10 work logs. The current host version is `null`/`UNPROBED` and the host is `UNSUPPORTED`; its internal `native-runtime-adapter` leaf maps to `NOT_APPLICABLE` with `HOST_UNSUPPORTED`, so any overall PASS is repository-only qualified. Supported-host matching requires `PROBED` version provenance. A supported-host snapshot can pass only after real Ed25519 verification, freshness and one-use challenge checks, and signed callback proof for all four surfaces; missing crypto and every trust fault remain completion-blocking.
