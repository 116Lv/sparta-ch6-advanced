...
| AI workflow verification completeness, task/change applicability, executable gate entry points, native runtime adapter state, or result mapping | `ai/verification-gates.md`, `ai/verification-policy.json`, `ai/native-runtime-adapters.md`, `ai/native-runtime-adapters.json`, `scripts/ai/verification-gate.sh`, relevant QA/work-log docs | Use for Phase 2C and Phase 3A repo-wide verification gate integration. The current host is `UNSUPPORTED`; `native-runtime-adapter` is evaluated in-process and supported-host faults remain completion-blocking. `scripts/ai/command-runner.sh` remains the only supported product-command path. Phase 3B owns CI and remote-runner enforcement. |
