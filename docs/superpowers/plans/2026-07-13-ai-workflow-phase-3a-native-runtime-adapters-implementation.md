...
Validate bypass-attempt fixtures against the closed schema. Add focused tests for mismatched `taskKey`, mismatched `gateInvocationId`, malformed records, redaction uncertainty, raw secret fields, summary byte/scalar bounds, repeated `eventId` idempotency, semantic `deduplicationKey` grouping without event deletion, `DETECTED`/`RESOLVED` transitions, unresolved completion blocking, exact signed current-resolution event binding, and command-based file/search intent retaining `surface: COMMAND`. A later-gate resolution may clear only for a supported/probed host with a fresh valid Ed25519 snapshot, trusted all-`ENFORCED` callbacks, and exact `resolutionEventIds`; missing, mismatched, duplicate, oversized, or extra binding and unsupported/no-snapshot claims block. Do not execute or spawn any command.
