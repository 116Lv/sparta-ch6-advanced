...
`.state/run-session.json` persists through OPEN, FINALIZING, and FINALIZED. Its closed `finalizationJournalIdentity` is null in OPEN, is the exact journal path/UUID/digest identity in FINALIZING, and remains equal to the FINALIZED receipt's `journalIdentity`. Its FINALIZED form is retained read-only as the authoritative finalization receipt and is excluded from the artifact manifest; `run.json` exists only after finalization publication. `.state/finalization-journal.json` is independent closed control authority containing its journal UUID, the exact source OPEN session, the exact intended FINALIZING session, claim-input reference, and expected lock-recovery prefix. It is excluded from the manifest and retained read-only after successful finalization. Repeated command attempts use collision-free result and log names containing an attempt identifier while preserving `commandId` inside JSON. Every schema-versioned immutable, session, or evidence artifact `$id` equals its repository-relative forward-slash reference. The transient `.state/lock/owner.json` is closed embedded-schema control JSON, is never an artifact or reference, and has no artifact `$id` equality requirement.
...
8. The finalizer schema-validates and publishes the receipt's exact `runProjection` as `run.json` last, then makes the retained FINALIZED session and journal read-only. A read-only `verify-finalized` operation recomputes directory closure and digests, loads the retained receipt and independent journal as authority, and emits its result outside the run; it never appends evidence to a finalized run.

An in-process validation or publication exception before step 8 invokes bounded rollback while the same validated run-lock owner is still held. Publication uncertainty, including runtime exceptions, is reconciled against the possibly published journal and current session and returns only a schema-valid structured result; it never escapes a traceback. Rollback first proves that `run.json` is absent, validates the independent journal, and proves that `.state/run-session.json` still equals its exact FINALIZING or receipt-bearing FINALIZED projection plus only an allowed lock-recovery suffix. It removes only the attributed `done-claim.json`, PRE_DONE_CLAIM gate result, and artifact manifest, restores the journal's exact OPEN source with the allowed suffix, and then removes/fsyncs the exact journal. A crash that leaves OPEN plus a journal blocks every normal operation. Explicit recovery alone may validate the exact OPEN source and allowed recovery suffix, remove/fsync the journal, and return `ALREADY_OPEN` without another normal mutation. FINALIZING recovery validates its top-level journal identity before cleanup or OPEN normalization. A changed journal ID/reference/digest, changed lock owner, changed sealed session, unsafe cleanup target, cleanup failure, or any appearance of `run.json` prevents rollback and returns `BLOCKED` with `FINALIZATION_RECOVERY_REQUIRED`, retaining the exact recovery state. Every failed recovery that observes raw FINALIZED state reapplies read-only mode to the session and retained journal before releasing the lock. After `run.json` publication, verification is anchored in the retained FINALIZED receipt, journal, and manifest digests, and a published `run.json` is never rolled back. No PRE_DONE_CLAIM operation reads a finalized `run.json` to construct its authority, so finalization has no circular trust dependency.
