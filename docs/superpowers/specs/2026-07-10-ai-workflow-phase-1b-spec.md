...
`.state/run-session.json` persists through OPEN, FINALIZING, and FINALIZED. Its closed `finalizationJournalIdentity` is null in OPEN, is the exact per-attempt journal path/UUID/digest identity in FINALIZING, and remains equal to the FINALIZED receipt's `journalIdentity`. Its FINALIZED form is retained read-only as the authoritative finalization receipt and is excluded from the artifact manifest; `run.json` exists only after finalization publication. Every `.state/finalization-journals/<journal-id>.json` is independent closed control authority containing its journal UUID, the exact source OPEN session, the exact intended FINALIZING session, claim-input reference, and expected lock-recovery prefix. Journals are excluded from the manifest, made read-only before visibility, and never deleted after publication; an inactive retained journal is audit history and a later attempt always creates a new UUID path. A fixed legacy `.state/finalization-journal.json` marker is not accepted and requires explicit fail-closed recovery. Repeated command attempts use collision-free result and log names containing an attempt identifier while preserving `commandId` inside JSON. Every schema-versioned immutable, session, or evidence artifact `$id` equals its repository-relative forward-slash reference. The transient `.state/lock/owner.json` is closed embedded-schema control JSON, is never an artifact or reference, and has no artifact `$id` equality requirement.
...
3. `done-claim-check.sh prepare <run-id> --claim <path>` creates a fresh UUID and exclusively publishes `.state/finalization-journals/<journal-id>.json`. The journal embeds the exact OPEN source and intended FINALIZING projection. Its identity digest is SHA-256 over compact canonical JSON after replacing only the embedded FINALIZING identity's `canonicalSha256` with 64 ASCII zeroes; this single normalization breaks the otherwise circular self-reference and every validator applies the same rule.
4. The first session compare-and-swap changes the exact journal source OPEN session to the journal's exact FINALIZING session, including `finalizationJournalIdentity`. No new command, approval, or other normal OPEN operation is accepted after that transition. Inactive unique audit journals do not block OPEN; an OPEN session with a non-null active identity or any fixed legacy marker is recovery-required.
5. PRE_DONE_CLAIM validates the FINALIZING session, all reservations, every discovered artifact, policy events, and the proposed done claim, then derives the complete gate result and candidate `run.json` in memory.
6. Before publishing final artifacts, the finalizer compare-and-swaps the exact FINALIZING session to FINALIZED with a closed `finalizationReceipt`. The receipt retains every schema-defined `run.json` field, canonical done-claim and gate digests plus outcomes, the exact manifest ID/run ID, the journal identity equal to the top-level session identity, and the complete expected artifact path/kind identity set.
7. The finalizer publishes the done claim and gate, requires exact evidence closure, and writes `artifact-manifest.json` with repository-relative path, SHA-256 digest, and byte size for every immutable artifact except the retained `.state`, manifest, and final `run.json` themselves.
8. Every immutable final JSON is fully encoded to a same-directory temporary file, file-fsynced, changed to read-only, and mode-verified before an exclusive link makes it visible; the containing directory is fsynced afterward. The finalizer publishes the receipt's exact `runProjection` as `run.json` last, then re-applies and verifies read-only mode on the exact claim, gate, manifest, run, FINALIZED session, and active journal before success. A read-only `verify-finalized` operation recomputes directory closure and digests, loads the retained receipt and independent journal as authority, and emits its result outside the run; it never appends evidence to a finalized run.

An in-process validation or publication exception before step 8 invokes bounded rollback while the same validated run-lock owner is still held. Publication uncertainty, including failures after temporary-file chmod or after the exclusive link, is reconciled against the possibly visible read-only artifact, active journal, and current session and returns only a schema-valid structured result; it never escapes a traceback. Rollback first proves that `run.json` is absent, validates the active independent journal, and proves that `.state/run-session.json` still equals its exact FINALIZING or receipt-bearing FINALIZED projection plus only an allowed lock-recovery suffix. It removes only the attributed `done-claim.json`, PRE_DONE_CLAIM gate result, and artifact manifest, then compare-and-swaps the journal's exact OPEN source with the allowed suffix. That OPEN CAS clears `finalizationJournalIdentity`; no later journal unlink or cleanup is part of the transaction. The immutable journal remains read-only audit history, normal OPEN work ignores inactive unique journals, and the next finalization attempt creates a distinct journal. An applied-error after the OPEN CAS is reconciled by rereading the exact session. FINALIZING recovery validates its top-level journal identity before cleanup or OPEN normalization. A fixed legacy marker, changed journal ID/reference/digest, changed lock owner, changed sealed session, unsafe cleanup target, cleanup failure, or any appearance of `run.json` prevents rollback and returns `BLOCKED` with `FINALIZATION_RECOVERY_REQUIRED`, retaining the exact recovery state. Successful resume and `ALREADY_FINALIZED` recovery re-apply and verify read-only mode on the claim, gate, manifest, run, session, and active journal; every failed recovery that observes raw FINALIZED state still repairs the session and safely identified journal before releasing the lock. After `run.json` publication, verification is anchored in the retained FINALIZED receipt, journal, and manifest digests, and a published `run.json` is never rolled back. No PRE_DONE_CLAIM operation reads a finalized `run.json` to construct its authority, so finalization has no circular trust dependency.
