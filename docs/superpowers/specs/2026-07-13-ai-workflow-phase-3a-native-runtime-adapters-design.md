...
Supported-host authority is supplied only to the internal evaluator as immutable `HostNativeTrust`. Its closed descriptor, authoritative probe, pinned producer and key fingerprint, and replay-ledger root must resolve outside the repository. Canonical repository policy keeps `supportedHosts` empty, and the public repository CLI accepts no descriptor, probe, ledger, policy, runtime snapshot, or bypass fixture capable of injecting supported-host authority; it therefore remains on the `UNSUPPORTED`/`UNPROBED` path.

Runtime discovery is a separate ephemeral snapshot closed by `ai/schemas/native-runtime-snapshot.schema.json` and consumed only after external host trust is established. Repository files cannot create or modify authoritative host trust and can never self-promote `ENFORCED`. Each future supported-host descriptor pins an Ed25519 public-key fingerprint and producer identifier. The snapshot supplies the raw public key; the evaluator hashes it against both the signed fingerprint and the external trust pin. It binds the current `taskKey`, a bounded `bypassEventCount`, and `bypassEventSetSha256`, computed over the complete deduplicated event objects sorted by `eventId` and encoded as compact UTF-8 JSON with sorted object keys. Its required `resolutionEventIds` member is a unique array of at most 64 identifiers, each at most 128 characters, and is empty when no current resolution is claimed. The host producer signs the RFC 8785-compatible restricted canonical JSON bytes of the snapshot without its `signature` member, so the task, event-set facts, and resolution IDs are inside the signed payload; the gate verifies the detached signature with optional `cryptography.hazmat`. The restricted subset permits only ASCII keys and string values, arrays, objects, booleans, null, and interoperable-range integers; floats and non-ASCII data are rejected. The private key remains host-owned and is never accepted from the repository, environment variables, command arguments, or the snapshot itself.

A signed snapshot is trusted only when its signature and producer identifier match external host trust, its host/version match the authoritative external probe, its adapter version is allowed, its signed `taskKey` and one-use `gateInvocationId` match the current challenge, its observation time is no more than 300 seconds old and not in the future, and every claimed blocking callback passes the signed host-producer pre-execution challenge result. Replay identity binds repository, producer, task, gate/challenge, attestation ID, nonce, and signed event-set digest. It is atomically consumed in the external host-owned ledger, with file and directory durability, before trusted surfaces can pass; the process-local set is defense in depth only. Unsafe handle-relative primitives or uncertain ledger publication, sync, close, or cleanup fail closed. After trust, the signed bypass count and digest must match the complete supplied event set before any PASS or resolution binding is accepted. Signature failure, unknown key, event-set mismatch, replay, callback failure, unavailable signature verification, or ledger uncertainty is `BLOCKED` for a supported host. When a valid later-gate `RESOLVED` transition exists, every current resolution `eventId` must match the signed `resolutionEventIds` set exactly; missing, mismatched, duplicate, oversized, or extra bindings block with a resolution-binding or snapshot-contract reason. Canonical `supportedHosts` remains empty, so the current host cannot pass and any resolution claim remains blocking. Temporary external trust, ledgers, and Ed25519 keys exercise the complete supported-host path and prove a valid signed all-`ENFORCED`, exactly bound resolution snapshot can produce `PASS`; no key, replay state, event-set state, resolution-binding state, or evidence becomes durable repository state.
...
- `operationType` and closed command intent classification. A native non-command surface must match its operation type and cannot carry `commandIntent`; a shell-mediated logical operation uses `surface: COMMAND` and a matching `commandIntent`.
- `statusAtObservation`.
...
- `detectionEventId`, `detectionGateInvocationId`, and `detectionEventSha256`: all null for `DETECTED` and all non-null for `RESOLVED`.
...
Tests validate schemas, external immutable host-trust paths, the public unsupported boundary, current-version provenance, restricted canonical bytes, real temporary Ed25519 verification, missing-crypto blocking, status transitions, discovery freshness, minimum versions, producer/key/signature/challenge/callback rejection, cross-process durable replay and ledger faults, claimed/trusted separation, hook-specific semantics, bypass normalization, original-detection binding, redaction, completion blocking, Phase 2C mapping, repository gateway authority, and current-host `UNSUPPORTED` behavior. All mutable artifacts are written under temporary directories outside the copied repository fixture.
...
Phase 3B owns CI adapter installation, CI fail-closed guarantees, remote-runner discovery, durable CI evidence, CI ledger provisioning and retention, required-check wiring, cross-host parity, and CI retention/attestation. Phase 3A provides only the local external-trust and durable-replay contract plus the helper/static enforcement boundary needed by that later work.
